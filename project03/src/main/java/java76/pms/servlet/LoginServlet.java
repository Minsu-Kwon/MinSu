...
public class LoginServlet extends HttpServlet {  
  ...
  public void doPost(
    ...
    try {
      ...
      if (saveEmail != null) { //이메일저장을 체크함. 따라서 값이 넘어온다.
        ...
        emailCookie.setMaxAge(0); //웹 브라우저에게 email쿠키 삭제를 명령한다.
      }
      response.addCookie(emailCookie);
      ...
      Student student = studentDao.login(email, password); //유저 요청

      HttpSession session = request.getSession();
      if (student == null) { //로그인 실패
        session.invalidate(); //세션 무효화시킴 => 새로 세션 객체 생성
